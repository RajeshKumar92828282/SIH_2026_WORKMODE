// Pure Node JS verification test for APIx 2-pass weight normalization math
...
function runTest() {
  ...
  // Assume +10% overall market surge
  const rawRows = staticFares.map(sf => {
    const staticTotal = sf.totalFare;
    const liveTotal = sf.totalFare * 1.10;
    const relativePrice = (liveTotal / staticTotal) * 100;
    return {
      ...sf,
      relativePrice,
